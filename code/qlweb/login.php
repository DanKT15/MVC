...
<div class="group-box">
	...
	if (isset($_POST["btnDangNhap"])){
		...
		if ($type == "sv"){
			$sql = "SELECT Ten , MatKhau FROM dbo_sinhvien WHERE Ten ='".$user."' AND MatKhau ='".$pass."'";
		}else{
			$sql = "SELECT Ten , MatKhau FROM dbo_giangvien WHERE Ten ='".$user."' AND MatKhau ='".$pass."'";
			// "SELECT MaGV as User, concat(Holot,' ', Ten) as HoTen, MatKhau  ".
		...
		if($row = $result->fetch_array()){	
			...
			if (isset($_POST["chkNhoMK"])){
				setcookie("User",$row["Ten"],time()+3600*24 );
